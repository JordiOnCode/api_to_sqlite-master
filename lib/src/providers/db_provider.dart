...
class DBProvider {
  ...
  initDB() async {
    ...
    return await openDatabase(path, version: 1, onOpen: (db) {},
      ...
      await db.execute('CREATE TABLE Information('
          'id TEXT PRIMARY KEY,'
          'name TEXT,'
          'boss TEXT,'
          'avatarboss TEXT,'
          'CreationData INTEGER'
          ')');
    });
  }
  ...
  createInformation(Information newInformation) async {
    ...
    final res = await db?.insert('Information', newInformation.toJson());
    return res;
  }
  ...
  Future<int?> deleteAllInformation() async {
    final db = await database;
    final res = await db?.rawDelete('DELETE FROM Information');
    return res;
  }

  Future<List<Information?>> getAllInformation() async {
    final db = await database;
    final res = await db?.rawQuery("SELECT * FROM INFORMATION");
